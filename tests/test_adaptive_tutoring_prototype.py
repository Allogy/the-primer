...
from examples.adaptive_tutoring_prototype import routine_graph
...
def test_route_next_node_rejects_unknown_type(
    ...
    present_step = next(
        step
        for step in invalid_routine["flow"]["steps"]
        if step["id"] == "present_exercise"
    )
